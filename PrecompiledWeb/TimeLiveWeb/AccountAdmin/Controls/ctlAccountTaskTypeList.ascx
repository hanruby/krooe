<%@ control language="VB" autoeventwireup="false" inherits="AccountAdmin_Controls_ctlAccountTaskTypeList, App_Web_j1ru2ref" %>
...
<x:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" DataKeyNames="AccountTaskTypeId"
    ...
    <Columns>
        <asp:TemplateField InsertVisible="False" SortExpression="AccountTaskTypeId">
            <edititemtemplate>
...
<asp:Label id="Label2" runat="server" Text='<%# Bind("TaskType") %>' __designer:wfdid="w57"></asp:Label> 
     ...
     <ItemTemplate>
<asp:LinkButton id="LinkButton1" runat="server" Text="<%$ Resources:TimeLive.Web, Delete_text %>" OnClientClick="<%# ResourceHelper.GetDeleteMessageJavascript() %>" CommandName="Delete" CausesValidation="False" __designer:wfdid="w5" Visible="<%# iif(me.gridview1.rows.count <= 0,0,1) %>"></asp:LinkButton> 
</ItemTemplate>
            <HeaderStyle HorizontalAlign="Center" />
            <ItemStyle HorizontalAlign="Center" Width="6%"  cssclass="delete_button"/>
        </asp:TemplateField>
        <asp:TemplateField HeaderText="<%$ Resources:TimeLive.Web, Disabled: %>">
            <HeaderTemplate>
        ...
        <table style="width: 100%" class="xview">
            <tr>
                <td class="caption" colspan="2" style="height: 21px">
                    <asp:Literal ID="Literal1" runat="server" Text='<%# ResourceHelper.GetFromResource("Task Type Information") %>' /></td>
            </tr>
            <tr>
                <td class="FormViewSubHeader" colspan="2" style="height: 21px">
                    <asp:Literal ID="Literal2" runat="server" Text='<%# ResourceHelper.GetFromResource("Task Type") %>' /></td>
            </tr>
            <tr>
...
<asp:Label ID="UserNameLabel" runat="server" AssociatedControlID="TaskTypeTextBox">
                  <asp:Literal ID="Literal5" runat="server" Text='<%# ResourceHelper.GetFromResource("Task Type:") %>' /></asp:Label></td><td style="width: 70%; height: 26px;">
        <asp:TextBox ID="TaskTypeTextBox" runat="server" Text='<%# Bind("TaskType") %>' MaxLength="50"></asp:TextBox><asp:RequiredFieldValidator ID="RequiredFieldValidator1" runat="server" ControlToValidate="TaskTypeTextBox"
            ...
            <tr>
                ...
                <td style="width: 70%; height: 26px;">
                    <asp:Button ID="Button1" runat="server" Text="<%$ Resources:TimeLive.Web, Update_text%> " CommandName="Update" OnClick="Button1_Click" Width="68px" /><asp:Button ID="Button2" runat="server" Text="<%$ Resources:TimeLive.Web, Cancel_text%> " CommandName="Cancel" Width="69px" /></td>
            </tr>
        </table>
    </EditItemTemplate>
    <InsertItemTemplate>
        <table style="width: 100%" class="xview">
            <tr>
                <td class="caption" colspan="2" style="height: 21px">
                  <asp:Literal ID="Literal3" runat="server" Text='<%# ResourceHelper.GetFromResource("Task Type Information") %>' /></td>
            </tr>
            <tr>
                <td class="FormViewSubHeader" colspan="2" style="height: 21px">
                    <asp:Literal ID="Literal4" runat="server" Text='<%# ResourceHelper.GetFromResource("Task Type") %>' /></td>
            </tr>
            <tr>
                <td style="height: 26px; width: 30%;" class="FormViewLabelCell" align="right">
                                        <SPAN 
