<%@ control language="VB" autoeventwireup="false" inherits="AccountAdmin_Controls_ctlAccountStatusList, App_Web_yhen42r4" %>
<asp:UpdatePanel ID="UpdatePanel1" runat="server">
    <ContentTemplate>
        <x:GridView ID="GridView1" runat="server" Width="98%" DataSourceID="dsAccountStatusObject" AutoGenerateColumns="False" DataKeyNames="AccountStatusId" SkinID="xgridviewSkinEmployee" Caption='<%# ResourceHelper.GetFromResource("Status List") %>'>
            <Columns>
                <asp:BoundField DataField="AccountStatusId" HeaderText="<%$ Resources:TimeLive.Web, Id %>" InsertVisible="False"
                    ReadOnly="True" SortExpression="AccountStatusId">
                    <ItemStyle Width="25px" />
                </asp:BoundField>
                <asp:TemplateField HeaderText="<%$ Resources:TimeLive.Web, Status Type %>"  SortExpression="SystemStatusType">
                    <edititemtemplate>
...
<asp:Label id="Label8" runat="server" Text='<%#GetGlobalResourceObject("TimeLive.Web", Eval("SystemStatusType"))%>' __designer:wfdid="w147"></asp:Label> 
                ...
                <asp:TemplateField SortExpression="Status" HeaderText="<%$ Resources:TimeLive.Web, Status %>">
                    <edititemtemplate>
...
<asp:Label id="Label9" runat="server" __designer:wfdid="w95" Text='<%# Bind("Status") %>'></asp:Label> 
                ...
                <asp:CommandField HeaderText="<%$ Resources:TimeLive.Web, Edit_text %>" SelectText="Edit" ShowSelectButton="True">
                    ...
                    <ItemStyle Width="3%" HorizontalAlign="Center" cssclass="edit_button"  />
                </asp:CommandField>
        <asp:TemplateField HeaderText="<%$ Resources:TimeLive.Web, Delete_text %>" ShowHeader="False">
     <ItemTemplate>
<asp:LinkButton id="LinkButton1" runat="server" __designer:wfdid="w1" Text="<%$ Resources:TimeLive.Web, Delete_text %>" CausesValidation="False" CommandName="Delete" Visible='<%# IIF(Isdbnull(Eval("MasterAccountStatusId")),True,False) %>' OnClientClick="<%# ResourceHelper.GetDeleteMessageJavascript() %>"></asp:LinkButton> 
</ItemTemplate>
            <HeaderStyle HorizontalAlign="Center" />
            <ItemStyle HorizontalAlign="Center" Width="3%" cssclass="delete_button"  />
                ...
                <asp:TemplateField>
                    <HeaderTemplate>
<asp:Image id="Image1" runat="server" __designer:wfdid="w3" ToolTip="<%$ Resources:TimeLive.Web, Disabled_text%> " ImageUrl="~/Images/Disabled.gif"></asp:Image> 
</HeaderTemplate>
                    <ItemTemplate>
<asp:Image id="Image1" runat="server" __designer:wfdid="w2" ToolTip="<%$ Resources:TimeLive.Web, Disabled_text%> " Visible='<%# IIF(IsDBNull(Eval("IsDisabled")),"false",Eval("IsDisabled")) %>' ImageUrl="~/Images/Disabled.gif"></asp:Image> 
</ItemTemplate>
                <ItemStyle Width="3%" cssclass="status_button" />
                </asp:TemplateField>
            </Columns>
        </x:GridView>
...
<asp:UpdatePanel ID="UpdatePanel2" runat="server">
    <ContentTemplate>
        <asp:FormView ID="FormView1" runat="server" DataSourceID="dsAccountStatusFormObject" DataKeyNames="AccountStatusId" DefaultMode="Insert" SkinID="formviewSkinEmployee" Width="350px" OnDataBound="FormView1_DataBound">
            <InsertItemTemplate>
                <table width="100%" class="xview">
                    <tr>
                        <td colspan="2" style="height: 21px" class="caption">
                            <asp:Literal ID="Literal1" runat="server" Text='<%# ResourceHelper.GetFromResource("Status Information")%> ' /></td>
                    </tr>
                    <tr>
                        <td colspan="2" class="formviewsubheader">
                            <asp:Literal ID="Literal2" runat="server" Text='<%# ResourceHelper.GetFromResource("Status")%> ' /></td>
                    </tr>
                    <tr>
                  ...
                  class=reqasterisk>*</SPAN> <asp:Literal ID="Literal3" runat="server" Text='<%# ResourceHelper.GetFromResource("Status Type:")%> ' /></td>
                        <td style="width: 50%">
                            <asp:DropDownList ID="DropDownList1" runat="server" SelectedValue='<%# Bind("StatusTypeId") %>' DataSourceID="dsStatusTypeObject" DataTextField="SystemStatusType" DataValueField="SystemStatusTypeId">
                            </asp:DropDownList>
...
<asp:Label ID="UserNameLabel" runat="server" AssociatedControlID="StatusTextBox"><asp:Literal ID="Literal4" runat="server" Text='<%# ResourceHelper.GetFromResource("Status:")%> ' /></asp:Label></td><td style="width: 50%">
                            <asp:TextBox ID="StatusTextBox" runat="server" Text='<%# Bind("Status") %>' MaxLength="45"></asp:TextBox><asp:RequiredFieldValidator ID="RequiredFieldValidator2" runat="server" ControlToValidate="StatusTextBox"
            ...
            <EditItemTemplate><table width="100%" class="xview">
                ...
                <tr>
                    <td colspan="2" class="formviewsubheader">
                        <asp:Literal ID="Literal6" runat="server" Text='<%# ResourceHelper.GetFromResource("Status")%> ' /></td>
                </tr>
                <tr>
                    <td style="width: 30%" align="right" class="formviewlabelcell">
                                                                <SPAN 
                  ...
                  class=reqasterisk>*</SPAN><asp:Label ID="UserNameLabel" runat="server" AssociatedControlID="StatusTextBox"><asp:Literal ID="Literal8" runat="server" Text='<%# ResourceHelper.GetFromResource("Status:")%> ' /></asp:Label></td><td style="width: 50%">
                        <asp:TextBox ID="StatusTextBox" runat="server" Text='<%# Bind("Status") %>' MaxLength="45"></asp:TextBox><asp:RequiredFieldValidator ID="RequiredFieldValidator3" runat="server" ControlToValidate="StatusTextBox"
                            Display="Dynamic" ErrorMessage="*"></asp:RequiredFieldValidator></td></tr><tr>
                ...
                <tr>
                    ...
                    <td style="width: 50%">
                        <asp:Button ID="Update" runat="server" CommandName="Update" Text="<%$ Resources:TimeLive.Web, Update_Text%> " Width="50px" /><asp:Button ID="Cancel" runat="server" CommandName="Cancel" Text="<%$ Resources:TimeLive.Web, Cancel_Text%> " Width="50px" /></td>
                </tr>
            </table>
            </EditItemTemplate>
            <ItemTemplate>
