<%@ control language="VB" autoeventwireup="false" inherits="WebReport_Reports_Controls_ctlMyReports, App_Web_3d3bkzq3" %>
    <div>
        <table width="98%">
            <tr>
                <td align="left" style="width: 100px; height: 21px">
        <asp:Button ID="Button1" runat="server" Text="<%$ Resources:TimeLive.Web, Add New Report%>" 
                        PostBackUrl="~/WebReport/ReportDesign.aspx" /></td>
            </tr>
        </table>
        <br />
        <x:GridView ID="GridView1" PageSize="500" runat="server" DataSourceID="dsAccountReportObject" SkinID="xgridviewskinemployee" AutoGenerateColumns="False" Width="98%" DataKeyNames="AccountReportId,AccountId" Caption="<%$ Resources:TimeLive.Web, My Reports %>">
            <Columns>
                <asp:TemplateField HeaderText="<%$ Resources:TimeLive.Web, Report Name %>"><EditItemTemplate>
...
<TABLE class="xTableWithoutBorder" width="98%"><TBODY><TR><TD style="WIDTH: 51px" rowSpan=2><asp:ImageButton id="ReportIconImage" runat="server" Width="40px" Height="40px" PostBackUrl='<%# Eval("AccountReportId", "~/WebReport/ShowReport.aspx?AccountReportId={0}") %>' __designer:wfdid="w1" Visible="False"></asp:ImageButton></TD><TD style="HEIGHT: 21px"><asp:HyperLink id="ReportNameHyperLink" runat="server" __designer:wfdid="w10" NavigateUrl='<%# Eval("AccountReportId", "~/WebReport/ShowReport.aspx?AccountReportId={0}") %>' CssClass="bolda">[ReportNameHyperLink]</asp:HyperLink></TD></TR><TR><TD><asp:Label id="ReportDescriptionLabel" Text='<%# Eval("ReportDescription") %>' runat="server" __designer:wfdid="w11"></asp:Label></TD></TR></TBODY></TABLE>
                ...
                <asp:TemplateField HeaderText="<%$ Resources:TimeLive.Web, Customize %>" >

<ItemStyle Width="4%" horizontalalign="Center"></ItemStyle>
<ItemTemplate>
<asp:HyperLink id="CustomizeHyperLink" runat="server" __designer:wfdid="w2" Visible="False" text="<%$ Resources:TimeLive.Web, Customize %>" NavigateUrl='<%# Eval("AccountReportId", "~/WebReport/ReportDesign.aspx?AccountReportId={0}") %>'></asp:HyperLink> 
</ItemTemplate>
</asp:TemplateField>
...
<ItemStyle Width="4%" horizontalalign="Center"></ItemStyle>
<ItemTemplate>
<asp:HyperLink id="PermissionHyperLink" runat="server" __designer:wfdid="w3" Visible="False" text="<%$ Resources:TimeLive.Web, Permissions %>" NavigateUrl='<%# Eval("AccountReportId", "~/WebReport/AccountReportPermission.aspx?AccountReportId={0}") %>'></asp:HyperLink> 
</ItemTemplate>
</asp:TemplateField>
...
<asp:LinkButton id="LinkButton1" runat="server" PostBackUrl='<%# Eval("AccountReportId", "~/WebReport/Reports/DeleteReport.aspx?AccountReportId={0}") %>' __designer:wfdid="w5" Visible='<%# IIF(isdbnull(Eval("AccountId")),"False","True") %>' CausesValidation="False" OnClientClick="<%# ResourceHelper.GetDeleteMessageJavascript() %>"></asp:LinkButton> 
                ...
                <asp:TemplateField ShowHeader="False" HeaderText="Copy">
                    <itemstyle horizontalalign="Center" width="38px" />
                    <itemtemplate>
<asp:HyperLink id="CopyHyperLink" runat="server" NavigateUrl='<%# Eval("AccountReportId", "~/WebReport/MyReports.aspx?AccountReportId={0}&IsCopy=True") %>' __designer:wfdid="w4">Copy</asp:HyperLink> 
</itemtemplate>
                </asp:TemplateField>
            </Columns>
        </x:GridView>
