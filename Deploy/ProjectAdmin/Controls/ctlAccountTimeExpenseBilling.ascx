<%@ control language="VB" autoeventwireup="false" inherits="ProjectAdmin_Controls_ctlAccountTimeExpenseBilling, App_Web_qfntt3rg" %>
    ...
    <ContentTemplate>
        ...
        <x:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" Caption='<%# ResourceHelper.GetFromResource("Time Expense Invoice List") %>' SkinID="xgridviewSkinEmployee" DataKeyNames="AccountTimeExpenseBillingId,IsPaid"
            ...
            <Columns>
                ...
                <asp:TemplateField SortExpression="AccountClientId">
                    <edititemtemplate>
...
<asp:Label id="Label4" runat="server" Text='<%# Bind("PartyName") %>' __designer:wfdid="w3"></asp:Label>
                ...
                <asp:TemplateField SortExpression="InvoiceNumber">
                    <edititemtemplate>
...
<asp:Label id="Label5" runat="server" Text='<%# Bind("InvoiceNumber") %>' __designer:wfdid="w12"></asp:Label>
                ...
                <asp:TemplateField SortExpression="InvoiceDate">
                    <edititemtemplate>
...
<asp:Label id="Label7" runat="server" Text='<%# Bind("Description") %>' __designer:wfdid="w18"></asp:Label>
                ...
                <asp:TemplateField HeaderText="<%$ Resources:TimeLive.Web, Amount %>" SortExpression="GrandTotal">
                    <edititemtemplate>
...
<asp:LinkButton id="LinkButton1" runat="server" OnClientClick="<%# ResourceHelper.GetDeleteMessageJavascript() %>" Visible='<%# IIF(Isdbnull(Eval("AccountTimeExpenseBillingId")),"False","True") %>' __designer:wfdid="w3" CausesValidation="False" CommandName="Delete"></asp:LinkButton> 
            ...
            <HeaderStyle HorizontalAlign="Center" width="1%" />
            <ItemStyle HorizontalAlign="Center" cssclass="delete_button" width="0%"/>
        </asp:TemplateField>
        <asp:TemplateField>
            <HeaderTemplate>
<asp:Image id="Image1" runat="server" ToolTip="<%$ Resources:TimeLive.Web, Disabled_text%> " ImageUrl="~/Images/Disabled.gif" __designer:wfdid="w11"></asp:Image> 
</HeaderTemplate>
            <ItemTemplate>
<asp:Image id="Image2" runat="server" ToolTip="<%$ Resources:TimeLive.Web, Disabled_text%> " ImageUrl="~/Images/Disabled.gif" Visible='<%# IIF(IsDBNull(Eval("IsDisabled")),"false",Eval("IsDisabled")) %>' __designer:wfdid="w10"></asp:Image> 
</ItemTemplate>
...
<asp:Label id="Label1" runat="server" Text='<%# Bind("AccountTimeExpenseBillingId") %>' __designer:wfdid="w12"></asp:Label> 
                    ...
                    <asp:Button ID="btnUpdate" runat="server" OnClick="btnUpdate_Click" Text="<%$ Resources:TimeLive.Web, Update %>" Width="75px" Visible="False" />
                                <asp:Button ID="btnAdd" runat="server" OnClick="btnAdd_Click" Text="<%$ Resources:TimeLive.Web, Add_text %>" Width="75px" /></td>
                        </tr>
                    </table>
