<%@ control language="VB" autoeventwireup="false" inherits="Controls_ctlAccountHolidayTypes, App_Web_j1ru2ref" %>
<%@ Register Assembly="System.Web.Extensions" Namespace="System.Web.UI" TagPrefix="asp" %>

<asp:UpdatePanel ID="UpdatePanel1" runat="server"><ContentTemplate>
<x:GridView id="GridView1" runat="server" OnRowDeleting="GridView1_RowDeleting" OnRowDeleted="GridView1_RowDeleted" OnRowCommand="GridView1_RowCommand" DataKeyNames="AccountHolidayTypeId" Width="98%" CssClass="TableView" Caption="<%$ Resources:TimeLive.Web, Holiday Type List%>" SkinID="xgridviewSkinEmployee" AllowSorting="True" DataSourceID="dsAccountHolidayTypeGrid" AutoGenerateColumns="False"><Columns>
<asp:BoundField DataField="AccountHolidayType" HeaderText="<%$ Resources:TimeLive.Web, Holiday Type %>" SortExpression="AccountHolidayType">
<ItemStyle Width="80%"></ItemStyle>
</asp:BoundField>
<asp:TemplateField><ItemTemplate>
<asp:HyperLink id="HyperLink1" runat="server" NavigateUrl='<%# Eval("AccountHolidayTypeId", "~/AccountAdmin/AccountHolidayTypeDetail.aspx?AccountHolidayTypeId={0}") %>' Text="<%$ Resources:TimeLive.Web, Holidays%>" Visible='<%# IIF(Isdbnull(Eval("AccountHolidayTypeId")),"False","True") %>' __designer:wfdid="w44"></asp:HyperLink>
</ItemTemplate>
...
<asp:Image id="Image1" runat="server" __designer:wfdid="w48" ImageUrl="~/Images/Disabled.gif" ToolTip="<%$ Resources:TimeLive.Web, Disabled_text%> "></asp:Image>
</HeaderTemplate>
<ItemTemplate>
<asp:Image id="Image1" runat="server" Visible='<%# IIF(IsDBNull(Eval("IsDisabled")),"false",Eval("IsDisabled")) %>' __designer:wfdid="w47" ImageUrl="~/Images/Disabled.gif" ToolTip="<%$ Resources:TimeLive.Web, Disabled_text%> "></asp:Image>
</ItemTemplate>
...
<asp:FormView id="FormView1" runat="server" DataKeyNames="AccountHolidayTypeId" Width="550px" SkinID="formviewSkinEmployee" DataSourceID="dsAccountHolidayTypeFormObject" OnItemCommand="FormView1_ItemCommand" BorderWidth="1px" BorderStyle="Solid" BorderColor="Black" DefaultMode="Insert" OnDataBound="FormView1_DataBound"><EditItemTemplate>
<TABLE style="WIDTH: 100%" class="xview"><TBODY><TR><TD class="caption" width="30%" colSpan=2><asp:Literal id="Literal1" runat="server" Text='<%# ResourceHelper.GetFromResource("Holiday Type Information")%> '></asp:Literal></TD></TR><TR><TD class="FormViewSubHeader" width="30%" colSpan=2><asp:Literal id="Literal3" runat="server" Text='<%# ResourceHelper.GetFromResource("Holiday Type")%> '></asp:Literal></TD></TR><TR><TD class="FormViewLabelCell" align=right width="30%">
<asp:Label ID="UserNameLabel" runat="server" AssociatedControlID="HolidayTypeNameTextBox">
<asp:Literal id="Literal4" runat="server" Text="<%$ Resources:TimeLive.Web, Holiday Type:%>"></asp:Literal></asp:Label></TD>
<TD width="70%"><asp:TextBox id="HolidayTypeNameTextBox" runat="server" Width="176px" Text='<%# Bind("AccountHolidayType") %>' MaxLength="200"></asp:TextBox> <asp:RequiredFieldValidator id="RequiredFieldValidator2" runat="server" Width="1px" Display="Dynamic" ErrorMessage="*" ControlToValidate="HolidayTypeNameTextBox"></asp:RequiredFieldValidator></TD></TR><TR><TD style="HEIGHT: 26px" class="FormViewLabelCell" align=right width="30%"><asp:Literal id="Literal5" runat="server" Text="<%$ Resources:TimeLive.Web, Disabled:%> "></asp:Literal></TD><TD width="70%"><asp:CheckBox id="IsDisabledCheckBox" runat="server" Width="168px" Checked='<%# Bind("IsDisabled") %>'></asp:CheckBox></TD></TR><TR><TD width="30%"></TD><TD width="70%"><asp:Button id="Button1" runat="server" Width="68px" Text="<%$ Resources:TimeLive.Web, Update_Text%> " CommandName="Update"></asp:Button><asp:Button id="Button2" runat="server" Width="68px" Text="<%$ Resources:TimeLive.Web, Cancel_Text%> " CommandName="Cancel"></asp:Button></TD></TR></TBODY></TABLE>
</EditItemTemplate>
<InsertItemTemplate>
<TABLE style="WIDTH: 100%" class="xview"><TBODY><TR><TD class="caption" colSpan=2>

<asp:Literal id="Literal1" runat="server" Text='<%# ResourceHelper.GetFromResource("Holiday Type Information")%> '></asp:Literal></asp:Label></TD></TR><TR><TD class="FormViewSubHeader" colSpan=2><asp:Literal id="Literal2" runat="server" Text='<%# ResourceHelper.GetFromResource("Holiday Type")%> '></asp:Literal></TD></TR><TR><TD class="FormViewLabelCell" align=right colSpan=1 rowSpan=1>
<asp:Label ID="Label1" runat="server" AssociatedControlID="HolidayTypeNameTextBox">
<asp:Literal id="Literal3" runat="server" Text="<%$ Resources:TimeLive.Web, Holiday Type:%>"></asp:Literal></TD></asp:Label><TD><asp:TextBox id="HolidayTypeNameTextBox" runat="server" Width="176px" Text='<%# Bind("AccountHolidayType") %>' MaxLength="200"></asp:TextBox> <asp:RequiredFieldValidator id="RequiredFieldValidator2" runat="server" Width="1px" Display="Dynamic" ErrorMessage="*" ControlToValidate="HolidayTypeNameTextBox"></asp:RequiredFieldValidator></TD></TR><TR><TD></TD><TD><asp:Button id="btnAdd" runat="server" Width="68px" Text="<%$ Resources:TimeLive.Web, Add_Text%> " CommandName="Insert"></asp:Button></TD></TR></TBODY></TABLE>
</InsertItemTemplate>
